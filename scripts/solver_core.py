...
def solve_complex_equation(eq_str, allow_numerical=True, latex=False):
 ...
 factor_terms = '，'.join([f'$${_l(f, True)} = 0$$' for f in factors if f.has(z)])
 header = (
 f"原方程：$${eq_latex} = 0$$\n\n"
 f"因式分解：$${fac_latex} = 0$$"
 )
 ...
 "[STEP] 因式分解求解步骤：\n"
 " 1. 设 f(z) = 0。\n"
 f" 2. 对表达式进行因式分解，得到：$${fac_latex} = 0$$。\n"
 f" 3. 分别令各因子等于0，得到候选方程：{factor_terms}。\n"
 " 4. 求解候选方程得到 z 的解，并代回验证。\n"
 )
 return f"{steps}\n{header}\n\n[PASS] 解析解（因式分解法）：\n{format_solution_list(valid_sols, latex)}"
 factor_terms = '，'.join([f"{_l(f, False)} = 0" for f in factors if f.has(z)])
 steps = (
 "[STEP] 因式分解求解步骤：\n"
 " 1. 设 f(z) = 0。\n"
 f" 2. 对表达式进行因式分解，得到：{factored} = 0。\n"
 f" 3. 分别令各因子等于0，得到候选方程：{factor_terms}。\n"
 " 4. 求解候选方程得到 z 的解，并代回验证。\n"
 )
 return f"{steps}\n[PASS] 解析解（因式分解法）：\n{format_solution_list(valid_sols, latex)}"
 ...
 if latex:
 eq_latex = _l(expr, True)
 solved_terms = '，'.join([f'$${_l(s, True)}$$' for s in sym_sols])
 header = f"原方程：$${eq_latex} = 0$$"
 steps = (
 "[STEP] 符号求解推导：\n"
 " 1. 写出待解方程 f(z)=0。\n"
 f" 2. 使用符号求解器对表达式进行求根，得到候选解：{solved_terms}。\n"
 " 3. 代回原方程验证每个候选解是否成立。\n"
 )
 return f"{steps}\n{header}\n\n[PASS] 解析解：\n{format_solution_list(valid_sols, latex)}"
 solved_terms = '，'.join([str(s) for s in sym_sols])
 steps = (
 "[STEP] 符号求解推导：\n"
 " 1. 写出待解方程 f(z)=0。\n"
 f" 2. 使用符号求解器对表达式进行求根，得到候选解：{solved_terms}。\n"
 ...
 " 2. 生成一组复数初始猜测点。\n"
 " 3. 对每个初始值用牛顿迭代逼近根。\n"
 f" 4. 找到 {len(sorted_roots)} 个不同收敛根，并进行去重。\n"
 )
...
def _to_markdown_output(text):
 """后处理器：将标记化文本转换为 Markdown 输出格式"""
 result = []
 for line in text.split('\n'):
 stripped = line.strip()
 if not stripped:
 ...
 for tag, md_tag in replacements:
 if stripped.startswith(tag):
 line = line.replace(tag, md_tag, 1)
 break
 if stripped.startswith('>'):
 line = '**判定：** ' + stripped[1:].lstrip()
...
def solve(raw_input, latex=True, markdown=False):
 """
 复变函数求解器主入口。
 ...
 markdown: bool — True 时输出 Markdown 格式结果（默认内部启用 LaTeX 公式）
 ...
 # Step 4: 分发到对应求解器并应用 LaTeX / Markdown 后处理
 if markdown:
 latex = True
 ...
 # 补强步骤输出
 result = _ensure_full_steps(result)

 # Markdown / LaTeX 后处理
 if markdown:
 result = _to_markdown_output(result)
 elif latex and result:
